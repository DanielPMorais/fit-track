...
export function WorkoutDayItem({ day }) {
  ...
  const workoutLink = `/workouts/${day.id}`;

  return (
    <div className={styles.itemWrapper}>
      <div className={styles.info}>
        <h4 className={styles.title}>{day.title} <span className={styles.description}>- {day.description}</span></h4>
        <p className={styles.lastCompleted}>Último treino concluído em: {day.lastCompleted}</p>
      </div>
